...
- New Pipeline -> Azure Repos Git -> Pick your repo -> Existing Azure Pipelines YAML file -> Select `build-client.yml`. Rename the Pipeline.
- New Pipeline -> Azure Repos Git -> Pick your repo -> Existing Azure Pipelines YAML file -> Select `rollout-client.yml`. Rename the Pipeline.
- Create the "Environments" they will be deployed to - this is an artifact of how Azure DevOps tracks them. The Environments should simply be empty. Environments -> New Environment. If you wish to put guards on your environments, then add them here in the "Approvals and Checks" tab. You may need to refresh the page to get the permission to do it. It's a bug in Azure Devops.
